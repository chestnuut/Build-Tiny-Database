...
RC readBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage){
	...
	if(isfound == 0){
		...
		//if isRead != PAGE_SIZE, read is not successful
		if(isRead != PAGE_SIZE){
			return RC_READ_NON_EXISTING_PAGE;
		}
		...
		return RC_SEEK_FILE_POSITION_FAILED;
	}
}
...
RC writeBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage){
	...
	int isfound = fseek(fHandle->mgmtInfo, offset, SEEK_SET);
	if(isfound==0){
		int isWrite = fwrite(memPage, sizeof(char), PAGE_SIZE, fHandle->mgmtInfo);
		//if isWrite != PAGE_SIZE, the page isn't write to the file, return error
		if (isWrite != PAGE_SIZE){
			return RC_WRITE_FAILED;
		}
...
RC appendEmptyBlock(SM_FileHandle *fHandle){
	...
	if (isfound == 0){
		...
		int isAppend = fwrite(emptyPage, sizeof(char), PAGE_SIZE, fHandle->mgmtInfo);
		if (isAppend != PAGE_SIZE){
			return RC_WRITE_FAILED;
		}
