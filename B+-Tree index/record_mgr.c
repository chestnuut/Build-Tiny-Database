...
/* We changed some featrues from our implementation in assignment 3 for better compatible with 
assignment 4.
// editted by zinuo Li
*/

const int MAX_NUMBER_OF_PAGES = 100;
const int ATTRIBUTE_SIZE = 15; // attribute size named 15
// This is custom data structure defined for making the use of Record Manager.
typedef struct RecordManager
{
	BM_PageHandle pageHandle; // Buffer Manager's PageHandle for using Buffer Manager to access Page files
	BM_BufferPool bufferPool;// Buffer Manager's Buffer Pool for using Buffer Manager	
	RID recordID;	// Record ID	
	Expr *condition; // the condition for scanning the records in the table
	int tuplesCount; // stores the total number of tuples in the table
	int freePage;// stores the location of first free page which has empty slots in table
	int scanCount;// stores the number of records scanned
} RecordManager;
...
extern RC createTable (char *name, Schema *schema)
{
	...
	// Initalize the Buffer Pool using LRU page replacement policy
	...
	int result, k;
	// Allocate page pointer memory
	// Set number of tuples to 0
	*(int*)pageHandle = 0; 

	// Increment page pointer by sizeof(int)
	pageHandle = pageHandle + sizeof(int);
	
	// Set the first page = 1
	*(int*)pageHandle = 1;
	...
	*(int*)pageHandle = schema->keySize;
	...
	for(k = 0; k < schema->numAttr; k++)
		...
		// Increment pointer by sizeof(int) because type length is an integer
	       	pageHandle = pageHandle + sizeof(int);
    	}
...
extern RC openTable (RM_TableData *rel, char *name)
{
	...
	// pass table's meta data to our custom record manager meta data structure
	...
	// Setting the initial pointer (0th location)
	pageHandle = (char*) recordManager->pageHandle.data;
	
	// Getting total number of tuples from the page file
...
// This function inserts a new record in the table referenced by "rel" and
// updates the 'record' parameter with the RID of the newly inserted record
extern RC insertRecord (RM_TableData *rel, Record *record)
{
	// get the meta data stored in the table
	RecordManager *recordManager = rel->mgmtData;	
	...
	// Get the size in bytes needed to store on record for the given schema
	...
	// Appending '+' as tombstone to indicate this is a new record and should be 
	// removed if space is less
	*slotPointer = '+';
...
// This function scans each record in the table and stores the result record 
...
/************************************************/
/******* RECORDS AND ATTRIBUTE FUNCTIONS ********/
/************************************************/
